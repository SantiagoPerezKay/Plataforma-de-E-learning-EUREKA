...
export const AccordionItem = ({title, content, idModule,idCurso}) => {
  ...
  const [contentIsSelected, setContentIsSelected] = useState(null)
  ...
  const handleContentSelected = (index) => {
    setContentIsSelected(index)
  }
        ...
        <div className="flex flex-col list-none text-xl">
          {content?.map(cont =>
              <NavLink to={`/dashboard/curso/${idCurso}/${idModule}/${cont.id}`}>
                <div onClick={()=> handleContentSelected(cont.id)} className={`flex items-center hover:bg-[#f1f1f1] h-20 ${cont.id == contentIsSelected? "border-l-4 border-l-blue-700" : "border-none"} /* ${cont.id == contentLocation? "border-l-4 border-l-blue-700" : "border-none"} */`}>
